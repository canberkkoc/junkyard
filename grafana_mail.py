...
def parse_args():
    ...
    parser.add_argument("-T", "--api_token",
                        ...
                        required=True)
    parser.add_argument("-H", "--timespan",
                        dest="time_span", type=str,
                        help="You must enter wished timespan like 24h 12h",
                        required=True)
...
async def download_async(session, panel, begin_date, end_date, grafana_server, api_token):
    url = f"{grafana_server}/render/d-solo/{panel[0]}/{panel[1]}?panelId={panel[2]}&orgId=1&from={begin_date}h \
          &to={end_date}&width=1334&height=750&tz=Europe/Istanbul"
    print("Istek URL:", url)
    async with session.get(url, headers={"Authorization": "Bearer " + api_token}) as resp:
        if resp.status == 200:
            ...
            with open(img_file, 'wb') as fd:
                while True:
                    chunk = await resp.content.read(20000)
                    if not chunk:
                        break
                    fd.write(chunk)
        else:
...
if __name__ == '__main__':
    ...
    if args.time_span is None:
        print("You must enter time span")
        exit(-1)
    # Determine timeframe to fetch for
    from_epoch, to_epoch = last_day(args.time_span)
    ...
    loop.run_until_complete(
        download_all_async(args.panel_list, from_epoch, to_epoch, args.grafana_server, args.api_token))
